# Canonical Directory Structure & Module Ownership Contract

**Project:** `MS-ADC` (Multimodal Semiconductor Defect Classification Agent)  
**Collaborators:** Kshitiz Khandelwal (`@Kshitiz-Khandel`) & Aditya Sharma (`@aditya`)  
**Standard Version:** v1.0.0  

---

## 📁 Repository Layout & File Contract

This directory layout is the **authoritative blueprint** for all feature branches and Pull Requests. Every module has a dedicated purpose, strict interface boundaries, and corresponding unit tests:
...
| PR # | Feature Branch | Primary Author | Reviewer | Scope & Files Covered |
| :---: | :--- | :---: | :---: | :--- |
| **PR #1** | `feat/infra-iac` | **Kshitiz** | Aditya | `terraform/` (VPC, BQ, GCS, IAM, Cloud Run), `tests/unit/test_infra_config.py` |
| **PR #2** | `feat/data-and-vfm` | **Aditya** | Kshitiz | `src/ingestion/`, `src/models/fine_tune_vfm.py`, `die_vfm.py`, `export_tensorrt.py`, `tests/unit/test_models.py` |
| **PR #3** | `feat/rag-knowledge-engine` | **Kshitiz** | Aditya | `data/fmea_corpus/`, `src/rag/`, `tests/unit/test_rag_retriever.py` |
| **PR #4** | `feat/agent-gateway-security`| **Kshitiz & Aditya**| Both | `src/models/wafer_vlm.py`, `src/gateway/`, `src/orchestrator/`, `src/security/`, `tests/unit/test_dlp_security.py` |
| **PR #5** | `feat/telemetry-ci-eval` | **Aditya & Kshitiz**| Both | `src/telemetry/`, `cloudbuild.yaml`, `src/evaluation/`, `tests/chaos/`, `docs/runbooks/` |
